...
export class EventBusService {
    ...
    connect(url: string, defaultHeaders: any = null, options: any = {}): void {
        let pingInterval: number = options.vertxbus_ping_interval || 5000;
        let pingTimerID: number;
        ...
        this.sockJS.onmessage = (e) => {
            let json: any = JSON.parse(e.data);
            ...
            if (this.handlers[json.address]) {
                ...
                let handlers: any = this.handlers[json.address];
     ...
     */
    publish(address: string, body: any, headers?: any) {
        if (this.connected) {
            let message: any = {
                address: address,
                body: body,
     ...
     * @param headers
     */
    register<T>(address: string, headers?: any): void {
        if (this.connected) {
            let envelope: any = {
                address: address,
                headers: mergeHeaders(this.defaultHeaders, headers),
     ...
     * @param headers
     */
    unregister<T>(address: string, headers?: any): void {
        if (this.connected) {
            let envelope: any = {
                address: address,
                headers: mergeHeaders(this.defaultHeaders, headers),
