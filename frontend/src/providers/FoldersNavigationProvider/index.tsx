...
export const FoldersNavigationProvider: FC<FoldersNavigationProviderProps> = ({
  ...
  const [currentFolder, setCurrentFolder] = useState<Folder>(useInitialCurrentFolder());
  const [folderData, setFolderData] = useState<Folder[]>([]);
  const [folderObject, setFolderObject] = useState<FolderObjectState>(initialFolderObject);
      ...
      (result: IFolderResponse[] | undefined, folderType: FOLDER_TYPE, title: string) => {
          if (result) {
              ...
              setFolderObject((prevFolderObject) => ({
                  ...prevFolderObject,
                  [folderType === FOLDER_TYPE.MY_BOARDS ? "myFolderObject" : "deletedFolderObject"]: folderObject,
              }));
          }
      },
      [t]
  ...
  useEffect(() => {
      if (triggerFetch.myFolders && myFoldersResult) {
          processFolders(myFoldersResult, FOLDER_TYPE.MY_BOARDS, "magneto.my.boards");
  ...
  useEffect(() => {
      if (triggerFetch.deletedFolders && deletedFoldersResult) {
          processFolders(deletedFoldersResult, FOLDER_TYPE.DELETED_BOARDS, "magneto.trash");
  ...
  const value = useMemo<FoldersNavigationContextType>(
      ...
      [currentFolder, folderData, folderObject, folders, folderNavigationRefs, getFolders, handleSelect]
  );
